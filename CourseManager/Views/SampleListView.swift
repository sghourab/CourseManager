...
struct SampleListView: View {
    ...
    var body: some View {
        ZStack {
            LinearGradient(gradient: Gradient(colors: [MainColors.accent, .blue, .purple]), startPoint: .top, endPoint: .bottomLeading).ignoresSafeArea()
            ScrollView {
                VStack {
                    overlayContent
                }
    ...
    var overlayContent: some View {
        ForEach(sampleWork, id: \.id) { sample in
            VStack(alignment: .leading, spacing: 12) {
                ...
                .tint(.accentColor)
                .controlSize(.large)
                .shadow(color: AppColors.shadow.opacity(0.2), radius: 5, x: 0, y: 1)
            }
            .padding(15)
            .background(
                Rectangle()
